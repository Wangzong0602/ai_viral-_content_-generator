...
访问：http://127.0.0.1:5454

> 端口说明：原计划 5173，但该端口落在 Windows 系统排除范围内无法绑定，改用 5454。
...
| 创作工作台 | `/` | 输入主题+选平台 → 一键生成（两步合一：自动采用第 1 个选题）→ SSE 实时进度+打字机 → 结果编辑/复制/导出 Markdown/纯文本 |
...
> 开发代理：`/api` 请求自动转发到后端 8001 端口（vite.config.js）。
